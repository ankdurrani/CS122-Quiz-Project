...
class QuizApp:
    ...
    def create_database(self):
        ...
        # Placeholder quizzes (remove later)
        c.execute("INSERT OR IGNORE INTO quizzes (id, topic) VALUES (1, 'Python Programming')")
        c.execute("INSERT OR IGNORE INTO quizzes (id, topic) VALUES (2, 'Geography')")
        c.execute("INSERT OR IGNORE INTO questions (quiz_id, question, answer_a, answer_b, answer_c, answer_d, correct) VALUES (1, 'What is Python?', 'A programming language', 'A snake', 'A type of coffee', 'No idea', 'A programming language')")
        c.execute("INSERT OR IGNORE INTO questions (quiz_id, question, answer_a, answer_b, answer_c, answer_d, correct) VALUES (1, 'Who created Python?', 'Guido van Rossum', 'Elon Musk', 'Bill Gates', 'Mark Zuckerberg', 'Guido van Rossum')")
        c.execute("INSERT OR IGNORE INTO questions (quiz_id, question, answer_a, answer_b, answer_c, answer_d, correct) VALUES (2, 'What is the capital of France?', 'Berlin', 'Fremont', 'Paris', 'Miami', 'Paris')")
    ...
    def launch_quiz_mode(self, quiz_id):
        ...
        conn = sqlite3.connect("quiz.db")
        c = conn.cursor()
        c.execute("SELECT question, answer_a, answer_b, answer_c, answer_d, correct FROM questions WHERE quiz_id = ?", (quiz_id,))
    ...
    def create_new_quiz(self):
        ...
        def save_quiz():
            ...
            for q in questions_list:
                c.execute('''
                    INSERT INTO questions (quiz_id, question, answer_a, answer_b, answer_c, answer_d, correct)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (quiz_id, *q))
